                                 ...
                                 )
{
  ...
  for ( unsigned ievent = 0; ievent < max_event; ievent++ )
      ...
      if ( ievent%1000 == 0 )
	{
      ...
      // Assume all Smeared Particles are not findable
       ...
       true_breakup_py.clear();
       true_breakup_pz.clear();
       true_breakup_pt.clear();
       true_breakup_p.clear();
       true_breakup_phi.clear();
       true_breakup_theta.clear();
       true_breakup_eta.clear();
       true_breakup_id.clear();
       ...
       for ( unsigned itrack = 0; itrack < ntracks; itrack++ )
	 {
	  ...
	  // Beam Hadron
	  else if(tparticle->Id().Code()!=11&&tparticle->Id().Code()!=22&&tparticle->GetStatus()==21)
	    {
	  ...
	  // Scattered Lepton
	  else if(tparticle->Id().Code()==11&&tparticle->GetStatus()==1&&tparticle->GetParentIndex()==3)
	    {
	   ...
	   // Scattered Hadron 
	  else if(tparticle->GetParentIndex()==1)
	    {
	  ...
	  else if(tparticle->GetParentIndex()==5)
	    {
  ...
  //----------------------------------------------------------------------------
  // Grab Cross Section from EICTree and Write Tree
  //----------------------------------------------------------------------------
  
  TVectorD *xsec;
  file_mc->GetObject("xsec",xsec);
  out_file->WriteObject(xsec,"xsec");
  out_tree->Write();
  out_file->Close();
  return 0;
}
